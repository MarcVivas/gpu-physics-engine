...
fn sort_test() {
    ...
    let sorter: GPUSorter = GPUSorter::new(device, guess_workgroup_size(wgpu_context).unwrap(), NonZeroU32::new(n).unwrap(), scrambled_keys_buffer.buffer(), scrambled_payload_buffer.buffer());
