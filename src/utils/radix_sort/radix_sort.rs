...
impl GPUSorter {
    ...
    /// # Arguments
    ///
    /// * `device` - The wgpu::Device to use for creating new buffers.
    ...
    pub fn create_sort_buffers <'a>(
        ...
        assert!(
            keys_a.size() >= (count_ru_histo * BYTES_PER_PAYLOAD_ELEM) as u64,
            "Provided keys buffer is too small."
        );

        // Ensure the user-provided payload buffer is large enough
        assert!(
            payload_a.size() >= (length * BYTES_PER_PAYLOAD_ELEM) as u64,
            "Provided payload buffer is too small."
        );


        // Create the auxiliary buffers required for the sorting passes
        ...
        let payload_b = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: (length * BYTES_PER_PAYLOAD_ELEM) as u64,
            usage: wgpu::BufferUsages::STORAGE,
            mapped_at_creation: false,
        });
